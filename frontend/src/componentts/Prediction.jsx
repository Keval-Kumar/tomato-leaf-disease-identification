...
export default function PredictionPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const scanEntry = {
        disease: DISEASE_DETAILS[key]?.displayName || key,
        confidence: Math.round((parsed.probability || 0) * 100),
        icon: DISEASE_DETAILS[key]?.emoji || "🍅",
        color: DISEASE_DETAILS[key]?.color || "#27ae60",
        date: new Date().toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }),
      };
      const existing = JSON.parse(localStorage.getItem("agroai_scan_history") || "[]");
      localStorage.setItem("agroai_scan_history", JSON.stringify([scanEntry, ...existing].slice(0, 100)));
      ...
      <header className={`fixed top-0 z-[100] w-full transition-all duration-300 ${scrolled ? "glass shadow-soft border-b border-white/50" : "bg-transparent"}`}>
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <div style={{
              display: "flex", alignItems: "center", gap: 6, background: "white", border: "1px solid rgba(45,80,22,0.12)", borderRadius: 999, padding: "5px 12px", fontSize: 11, fontFamily: "sans-serif", fontWeight: 600,
      ...
      <AnimatePresence>
        {result && diseaseKey && !translating && (
          <motion.button
            ...
            style={{
              position: "fixed", bottom: 28, right: 28, zIndex: 150, height: 56, borderRadius: 999,
