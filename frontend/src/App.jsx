...
// const features = [
//   { title:"Tomato Cultivation Guide", description:"A practical growth companion with seasonal guidance, field-ready tips, and visual planting support for healthier harvests.", route:"/tomato-cultivation", icon:<Trees className="h-5 w-5"/>, accent:"from-[#D4A574] to-[#B8C4A8]", bullets:["Planting stages","Soil & irrigation","Seasonal care"], sceneType:"sunrise" },
//   { title:"Disease Database", description:"Browse disease categories, symptoms, prevention notes, and sample visuals in a structured knowledge hub.", route:"/diseases", icon:<Database className="h-5 w-5"/>, accent:"from-[#7FD14F] to-[#B8C4A8]", bullets:["Symptoms","Treatment","Prevention"], sceneType:"disease" },
//   { title:"Model Information", description:"Explore the model pipeline, training overview, class mapping, and deployment notes — now powered by Claude AI vision.", route:"/model-info", icon:<Brain className="h-5 w-5"/>, accent:"from-[#2D5016] to-[#1a3a0a]", bullets:["Claude AI","Vision model","Deployment"], sceneType:"neural" },
//   { title:"News & Updates", description:"A clean space for agriculture news, weather guidance, and actionable updates for tomato growers.", route:"/news", icon:<Newspaper className="h-5 w-5"/>, accent:"from-[#2FA0D8] to-[#4DB8E8]", bullets:["Farm updates","Weather alerts","Policy news"], sceneType:"market" },
// ];

// // ═══════════════════════════════════════════════════════════════════════════════
// // 3D ANIMATED HERO CANVAS — Multi-layer parallax agricultural scene
// // ═══════════════════════════════════════════════════════════════════════════════
...
//       // ── Sky gradient — animated between dawn and noon ──
...
//       // ── Perspective lines (rows) — parallax mid ──
...
//       // ── Crop plants — parallax foreground ──
...
//           // Tomato fruits (larger plants only)
//           if (tD > 0.5 && pi % 2 === 0) {
//             ctx.beginPath();
...
// // ─── CANVAS SCENES (for feature cards) ───────────────────────────────────────
...
//           <button onClick={()=>window.location.href="/"} style={{display:"flex",alignItems:"center",gap:12,background:"none",border:"none",cursor:"pointer"}}><span style={{width:42,height:42,borderRadius:"50%",background:"linear-gradient(135deg,#2D5016,#1a3a0a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>🍅</span><span><span style={{display:"block",fontSize:10,fontWeight:700,letterSpacing:"0.35em",color:"rgba(45,80,22,0.7)",textTransform:"uppercase",fontFamily:"sans-serif"}}>Agro AI</span><span style={{display:"block",fontSize:14,fontWeight:600,color:"#1a3a0a",fontFamily:"sans-serif"}}>Tomato Disease Identifier</span></span></button>
...
//         {/* ── HERO — full animated 3D agricultural canvas ── */}
//         <section className="relative min-h-[calc(100vh-72px)] overflow-hidden">
//           {/* Animated 3D canvas background */}
...
//           {/* Floating decorative elements — parallax via CSS */}
...
//               transform:`translate(${(0.5-mouse.x)*-18}px,${(0.5-mouse.y)*-12}px)`,
...
//               filter:"drop-shadow(0 4px 12px rgba(127,209,79,0.5))",
//               animation:"float-y 5.5s ease-in-out 1s infinite",
//               transform:`translate(${(0.5-mouse.x)*-28}px,${(0.5-mouse.y)*-18}px)`,
//               transition:"transform 0.3s ease",
//             }}>🌿</div>
//             {/* Floating sparkle */}
//             <div style={{
...
//               fontSize:"22px",
//               filter:"drop-shadow(0 2px 8px rgba(249,168,37,0.6))",
//               animation:"float-y 3.8s ease-in-out 0.5s infinite",
//               transform:`translate(${(0.5-mouse.x)*-35}px,${(0.5-mouse.y)*-22}px)`,
//               transition:"transform 0.3s ease",
//             }}>✨</div>
//           </div>
...
//                 Claude AI Vision · No server required · 10 disease classes
//               </motion.div>

//               {/* Headline */}
//               <motion.h1
//                 className="max-w-4xl font-serif text-5xl font-bold leading-[0.98] tracking-tight text-white text-3d sm:text-6xl lg:text-7xl xl:text-[5.5rem]"
...
//                 {[["94%+","Accuracy"],["11 classes","Tomato diseases"],["Dark/Blur","image checks"]].map(([value,label],idx)=>(
...
//               <h2 className="mt-3 font-serif text-3xl font-bold text-[#1a3a0a]">AGRO AI — Integrate and Analyze with our Existing AI model</h2>
//               <p className="mt-4 text-sm leading-7 text-[#2C2C2C]/75">Upload a leaf image and AGRO AI analyses it directly. Pure AI vision, instant results along with image validation.</p>
//               <div className="mt-6">
//                 <button onClick={()=>window.location.href="/predictions"}
...
//                 <motion.div initial={{opacity:0,x:18}} whileInView={{opacity:1,x:0}} viewport={{once:true}}
//                   className="glass shadow-soft rounded-[2rem] border border-white/70 p-6 sm:p-8">
//                   <div className="flex items-center gap-3 text-sm font-semibold text-[#1a3a0a]"><Brain className="h-4 w-4 text-[#7c3aed]"/>AGRI AI Vision</div>
//                   <p className="mt-3 text-sm leading-7 text-[#2C2C2C]/75">Uses AI model's vision model to classify 10 tomato diseases directly from the image — with required treatment recommendations.</p>
//                 </motion.div>
//                 <motion.div initial={{opacity:0,x:18}} whileInView={{opacity:1,x:0}} viewport={{once:true}}
//                   className="glass shadow-soft rounded-[2rem] border border-white/70 p-6 sm:p-8">
...
//                     {[["#7FD14F","AI rejects poor-quality images"],["#E63946","Clear confidence display"],["#D4A574","Professional agriculture theme"]].map(([c,t])=>(
...
//               <motion.div initial={{opacity:0,x:18}} whileInView={{opacity:1,x:0}} viewport={{once:true}}
//                 className="glass shadow-soft rounded-[2rem] border border-white/70 p-6 sm:p-8">
//                 <div className="flex items-center gap-3 text-sm font-semibold text-[#1a3a0a]"><BarChart3 className="h-4 w-4 text-[#E63946]"/>Fast implementation</div>
//                 <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-[#2C2C2C]/75">
//                   {[["React","Frontend shell"],["AGRO AI","disease identifier"],["Vite","Build system"],["Tailwind","Design system"]].map(([a,b])=>(
...
//                           <div className="rounded-full bg-[#F5F1E8] px-3 py-1 text-xs font-semibold text-[#2D5016]">Section {index + 1}</div>
//                         </div>
...
//                 {["Claude AI","Responsive","Multi-language"].map(t=>(
